...
export function TabsTab({ className, ...props }) {
  return (
    <TabsPrimitive.Tab
      className={cn(
        'relative flex h-9 shrink-0 grow cursor-pointer items-center justify-center gap-1.5 whitespace-nowrap rounded-md border border-transparent px-2.5 font-medium text-base outline-none transition-[color,background-color,box-shadow] hover:text-muted-foreground focus-visible:ring-2 focus-visible:ring-ring data-[orientation=vertical]:w-full data-[orientation=vertical]:justify-start data-[active]:text-foreground disabled:pointer-events-none disabled:opacity-50 sm:h-8 sm:text-sm [&_svg:not([class*="size-"])]:size-4.5 sm:[&_svg:not([class*="size-"])]:size-4 [&_svg]:pointer-events-none [&_svg]:-mx-0.5 [&_svg]:shrink-0',
        className,
      )}
      data-slot="tabs-tab"
      {...props}
