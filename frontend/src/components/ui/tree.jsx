...
const TreeContext = React.createContext({ indent: 20, currentItem: undefined, tree: undefined })

function useTreeContext() {
  return React.useContext(TreeContext)
}

export function Tree({ indent = 20, tree, className, ...props }) {
  ...
  const mergedStyle = { ...propStyle, '--tree-indent': `${indent}px` }
...
export function TreeItem({ item, className, children, ...props }) {
...
export function TreeItemLabel({ item: propItem, children, className, ...props }) {
  ...
  return (
    <span
      ...
      className={cn(
        'bg-background hover:bg-accent in-data-[selected=true]:bg-accent in-data-[selected=true]:text-accent-foreground in-data-[drag-target=true]:bg-accent flex items-center gap-1 rounded-sm px-2 py-1.5 text-sm transition-colors not-in-data-[folder=true]:ps-7 [&_svg]:pointer-events-none [&_svg]:shrink-0',
        className,
      )}
      {...props}
    >
      {item.isFolder() && (
        <ChevronDownIcon className="text-muted-foreground size-4 in-aria-[expanded=false]:-rotate-90 transition-transform duration-150" />
      )}
      {children ||
...
export function TreeDragLine({ className, ...props }) {
